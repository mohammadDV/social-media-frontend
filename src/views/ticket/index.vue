...
//   const hasDeletePermission = ref(authStore.permissions.includes('ticket_delete'));  
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/tickets', serverOptions.value)
    .then((response: any) => {
...
//         useApi().deleteRequest(`/api/profile/tickets/${id}`)
//         .then((response: any) => {
//             if (response.data.status) {
...
//   };
  ...
  onMounted(() => {
    ...
    window.document.title =   `${t('site.Ticket management')} | ${t('site.Website name')}`;
  })
            ...
            <div class="card-body">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ $t('site.Ticket management') }}
                        </li>
                    </ol>
                </nav>
                <div v-if="hasShowPermission" class="place-button">
                    <router-link to="/profile/tickets/create" :title="$t('site.Create new ticket')">
                        <button class="btn btn-primary">{{ $t('site.Create new ticket') }}</button>
                    </router-link>
                </div>
            </div>
        </div>
