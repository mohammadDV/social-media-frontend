<script lang="ts" setup>
  ...
  const headers: Header[] = [
    ...
    { text: t('site.Place of advertise'), value: "place" },
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/advertise', serverOptions.value)
    .then((response: any) => {
  ...
  const deletItem = (id: Number) => {
    if(confirm('Are you sure you want to remove this item?')) {
        useApi().deleteRequest(`/api/profile/advertise/${id}`)
        .then((response: any) => {
