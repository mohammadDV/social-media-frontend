<script lang="ts" setup>
  ...
  const hasShowPermission = ref(authStore.permissions.includes('player_show'));
  const hasUpdatePermission = ref(authStore.permissions.includes('player_update'));
  const hasDeletePermission = ref(authStore.permissions.includes('player_delete'));  
  const { t } = useI18n();  
  const dataTable = ref();
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/players', serverOptions.value)
    .then((response: any) => {
  ...
  const deletItem = (id: Number) => {
    if(confirm('Are you sure you want to remove this item?')) {
        useApi().deleteRequest(`/api/profile/players/${id}`)
        .then((response: any) => {
  ...
  onMounted(() => {
    window.document.title =   `${t('site.Player management')} | ${t('site.Website name')}`;
  })
            ...
            <div class="card-body">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ $t('site.Player management') }}
                        </li>
                    </ol>
                </nav>
                <div v-if="hasShowPermission" class="place-button">
                    <router-link to="/profile/players/create" :title="$t('site.Create new player')">
                        <button class="btn btn-primary">{{ $t('site.Create new player') }}</button>
                    </router-link>
                </div>
            </div>
        </div>
        <div class="card p-3">
            ...
            <EasyDataTable
                ...
                <template #item-actions="item">
                    <div class="flex">
                        <router-link v-if="hasUpdatePermission" class="p-1 rounded btn-info m-1 text-white" :to="'/profile/players/' + item.id">
