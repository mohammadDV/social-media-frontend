<script lang="ts" setup>
  ...
  const hasShowPermission = ref(authStore.permissions.includes('club_show'));
  const hasUpdatePermission = ref(authStore.permissions.includes('club_update'));
  const hasDeletePermission = ref(authStore.permissions.includes('club_delete'));  
  const { t } = useI18n();  
  const dataTable = ref();
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/clubs', serverOptions.value)
    .then((response: any) => {
  ...
  const deletItem = (id: Number) => {
    if(confirm('Are you sure you want to remove this item?')) {
        useApi().deleteRequest(`/api/profile/clubs/${id}`)
        .then((response: any) => {
            ...
            <div class="card-body">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ $t('site.Club management') }}
                        </li>
                    </ol>
                </nav>
                <div v-if="hasShowPermission" class="place-button">
                    <router-link to="/profile/clubs/create" :title="$t('site.Create new club')">
                        <button class="btn btn-primary">{{ $t('site.Create new club') }}</button>
                    </router-link>
                </div>
            </div>
        </div>
        <div class="card p-3">
            ...
            <EasyDataTable
                ...
                <template #item-actions="item">
                    <div class="flex">
                        <router-link v-if="hasUpdatePermission" class="p-1 rounded btn-info m-1 text-white" :to="'/profile/clubs/' + item.id">
