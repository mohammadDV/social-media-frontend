<script lang="ts" setup>
  ...
  const hasDeletePermission = ref(authStore.permissions.includes('live_delete')); 
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/lives', serverOptions.value)
    .then((response: any) => {
  ...
  const deletItem = (id: Number) => {
    if(confirm('Are you sure you want to remove this item?')) {
        useApi().deleteRequest(`/api/profile/lives/${id}`)
        .then((response: any) => {
  ...
  onMounted(() => {
    ...
    window.document.title =   `${t('site.Live management')} | ${t('site.Website name')}`;
  }) 
            ...
            <div class="card-body">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ $t('site.Live management') }}
                        </li>
                    </ol>
                </nav>
                <div v-if="hasShowPermission" class="place-button">
                    <router-link to="/profile/lives/create" :title="$t('site.Create new live')">
                        <button class="btn btn-primary">{{ $t('site.Create new live') }}</button>
                    </router-link>
                </div>
            </div>
        </div>
        <div class="card p-3">
            ...
            <EasyDataTable
                ...
                <template #item-actions="item">
                    <div class="flex">
                        <router-link v-if="hasUpdatePermission" class="p-1 rounded btn-info m-1 text-white" :to="'/profile/lives/' + item.id">
