...
//   const hasUpdatePermission = ref(authStore.permissions.includes('advertise_update'));
  ...
  const headers: Header[] = [
    ...
    { text: t('site.Last name'), value: "last_name", sortable: true },
    { text: t('site.Phone'), value: "phone", sortable: true },
    { text: t('site.Description'), value: "content" },
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/advertise-form', serverOptions.value)
    .then((response: any) => {
  ...
  const deletItem = (id: Number) => {
    if(confirm('Are you sure you want to remove this item?')) {
        useApi().deleteRequest(`/api/profile/advertise-form/${id}`)
        .then((response: any) => {
            ...
            <div class="card-body">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ $t('site.Advertise form management') }}
                        </li>
                    </ol>
                </nav>
            </div>
