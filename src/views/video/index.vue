<script lang="ts" setup>
  ...
  const hasShowPermission = ref(authStore.permissions.includes('video_show'));
  const hasUpdatePermission = ref(authStore.permissions.includes('video_update'));
  const hasDeletePermission = ref(authStore.permissions.includes('video_delete'));
  ...
  const loadFromServer = async () => {
    loading.value = true;
    await useApi().get('/api/profile/videos', serverOptions.value)
    .then((response: any) => {
  ...
  const deletItem = (id: Number) => {
    if(confirm('Are you sure you want to remove this item?')) {
        useApi().deleteRequest(`/api/profile/videos/${id}`)
        .then((response: any) => {
  ...
  onMounted(() => {
    window.document.title =   `${t('site.Video management')} | ${t('site.Website name')}`;
            ...
            <div class="card-body">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ $t('site.Video management') }}
                        </li>
                    </ol>
                </nav>
                <div v-if="hasShowPermission" class="place-button">
                    <router-link to="/profile/videos/create" :title="$t('site.Create new video')">
                        <button class="btn btn-primary">{{ $t('site.Create new video') }}</button>
                    </router-link>
                </div>
            </div>
        </div>
        <div class="card p-3">
            ...
            <EasyDataTable
                ...
                <template #item-actions="item">
                    <div class="flex">
                        <router-link v-if="hasUpdatePermission" class="p-1 rounded btn-info m-1 text-white" :to="'/profile/videos/' + item.id">
