 ...
 const form = reactive({ ...initialFormState });
  ...
  const getItem = () => {
    ...
    if (route.params.id != undefined) {
        useApi().get(`/api/profile/sports/${route.params.id}`)
            .then((response) => {
  ...
  const send = () => {
    ...
    let url = '/api/profile/sports/';

    if (route.params.id) {
        url = `/api/profile/sports/${route.params.id}`;
    }
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">
                            {{ $t('site.Create new sport') }}
                        </li>
                    </ol>
                </nav>
                <div class="place-button">
                    <router-link to="/profile/sports" :title="$t('site.Sport management')">
                        <button class="btn btn-primary">{{ $t('site.Sport management') }}</button>
                    </router-link>
                </div>
            </div>
        </div>
